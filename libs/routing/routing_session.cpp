...
void RoutingSession::RemoveRoute()
{
  ...
  m_speedCameraManager.SetRoute(m_route);
  m_upcomingSpeedLimitProvider.SetRoute({});
  m_lastGpsSpeedMps = -1.0;
}
...
SessionState RoutingSession::OnLocationPositionChanged(GpsInfo const & info)
{
  ...
  if (info.HasSpeed())
    m_lastGpsSpeedMps = info.m_speed;
...
void RoutingSession::GetRouteFollowingInfo(FollowingInfo & info) const
{
  ...
  // Predictive Speed Limit / Eco Driving (advisory only).
  if (m_advisorConfig.m_enabled && m_lastGpsSpeedMps >= 0.0)
  {
    double const passedDist = m_passedDistanceOnRouteMeters + m_route->GetCurrentDistanceFromBeginMeters();
    std::vector<SpeedLimitChange> changes;
    m_upcomingSpeedLimitProvider.Collect(passedDist, m_advisorConfig.m_horizonMeters, changes);

    auto const prediction = PredictiveDrivingAdvisor::Advise(m_lastGpsSpeedMps, 0.0 /* altitude */, changes,
                                                             m_advisorConfig);
    info.m_ecoHasData = prediction.m_hasData;
    info.m_ecoRecommendation = prediction.m_recommendation;
    info.m_upcomingSpeedLimits = std::move(changes);
    info.m_ecoIsSequence = prediction.m_isSequence;
  }
}

void RoutingSession::SetEcoDrivingConfig(AdvisorConfig const & config)
{
  CHECK_THREAD_CHECKER(m_threadChecker, ());
  m_advisorConfig = config;
}
...
void RoutingSession::AssignRoute(std::shared_ptr<RoutesResult> const & result, RouterResultCode e)
{
  ...
  m_speedCameraManager.Reset();
  m_speedCameraManager.SetRoute(m_route);
  m_upcomingSpeedLimitProvider.SetRoute(m_route);
}
