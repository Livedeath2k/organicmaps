...
#include "routing/predictive_driving_advisor.hpp"  // EcoRecommendation
...
public:
  ...
  //@{
  // True when there is reliable explicit OSM data to show. When false the UI must
  // stay silent and must not display any numbers or recommendation.
  bool m_ecoHasData = false;

  // The eco driving recommendation (KeepSpeed / Coast / IncreaseRegen / ...).
  EcoRecommendation m_ecoRecommendation = EcoRecommendation::KeepSpeed;

  // Upcoming speed limit changes, nearest first. Speeds in km/h, distances in meters.
  std::vector<SpeedLimitChange> m_upcomingSpeedLimits;

  // True when several limits follow each other within a short distance, so the
  // driver should not re-accelerate between them.
  bool m_ecoIsSequence = false;
  //@}
};
}  // namespace routing
