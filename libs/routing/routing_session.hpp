...
public:
  ...
  /// Configure the Predictive Speed Limit / Eco Driving feature. When
  /// |config.m_enabled| is false (the default) the feature produces no output and
  /// has no side effects on navigation or routing.
  void SetEcoDrivingConfig(AdvisorConfig const & config);
...
private:
  ...
  UpcomingSpeedLimitProvider m_upcomingSpeedLimitProvider;
  AdvisorConfig m_advisorConfig;
  double m_lastGpsSpeedMps = -1.0;  // Last known GPS speed in m/s, or -1 if unknown.
