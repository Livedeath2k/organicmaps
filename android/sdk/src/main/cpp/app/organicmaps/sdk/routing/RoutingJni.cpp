...
jobject ToJavaEcoRecommendation(JNIEnv * env, routing::EcoRecommendation recommendation)
{
  static jclass const clazz = jni::GetGlobalClassRef(env, "app/organicmaps/sdk/routing/EcoRecommendation");
  char const * name = nullptr;
  switch (recommendation)
  {
    case routing::EcoRecommendation::KeepSpeed: name = "KeepSpeed"; break;
    case routing::EcoRecommendation::Coast: name = "Coast"; break;
    case routing::EcoRecommendation::IncreaseRegen: name = "IncreaseRegen"; break;
    case routing::EcoRecommendation::BrakeEarlier: name = "BrakeEarlier"; break;
    case routing::EcoRecommendation::BrakeNow: name = "BrakeNow"; break;
    case routing::EcoRecommendation::ReduceAccelerationTarget: name = "ReduceAccelerationTarget"; break;
  }
  return ToJavaEnum(env, clazz, "Lapp/organicmaps/sdk/routing/EcoRecommendation;", name);
}

jobjectArray CreateSpeedLimitChangeArray(JNIEnv * env, std::vector<routing::SpeedLimitChange> const & changes)
{
  using namespace jni;
  ...
  static jclass const clazz = GetGlobalClassRef(env, "app/organicmaps/sdk/routing/SpeedLimitChange");
  // Java signature : SpeedLimitChange(double distanceMeters, int speedKmph)
  static jmethodID const ctorId = GetConstructorID(env, clazz, "(DI)V");

  auto const size = static_cast<jsize>(changes.size());
  jobjectArray arr = env->NewObjectArray(size, clazz, nullptr);
  ASSERT(arr, (DescribeException()));

  for (jsize i = 0; i < size; ++i)
  {
    // Convert m/s to km/h for the Java layer.
    int const speedKmph = static_cast<int>(changes[i].m_speedMps * 3.6 + 0.5);
    jobject obj = env->NewObject(clazz, ctorId, changes[i].m_distFromCurrentMeters, speedKmph);
    ASSERT(obj, (DescribeException()));
...
jobject CreateRoutingInfo(JNIEnv * env, routing::FollowingInfo const & info, RoutingManager & rm)
{
  ...
  static jmethodID const ctorRouteInfoID = GetConstructorID(env, klass,
    ...
    "[Lapp/organicmaps/sdk/routing/LaneInfo;"                  // lanes
     ...
     "Z"                                                        // ecoHasData
     "Lapp/organicmaps/sdk/routing/EcoRecommendation;"          // ecoRecommendation
     "[Lapp/organicmaps/sdk/routing/SpeedLimitChange;"          // upcomingSpeedLimits
     "Z"                                                        // ecoIsSequence
     ")V"
   );
  // clang-format on
  ...
  jobject const result = env->NewObject(klass, ctorRouteInfoID,
    ...
    CreateLanesInfo(env, info.m_lanes),
     ...
     static_cast<jboolean>(info.m_ecoHasData),
     ToJavaEcoRecommendation(env, info.m_ecoRecommendation),
     CreateSpeedLimitChangeArray(env, info.m_upcomingSpeedLimits),
     static_cast<jboolean>(info.m_ecoIsSequence)
   );
  // clang-format on
  ASSERT(result, (DescribeException()));
  return result;
