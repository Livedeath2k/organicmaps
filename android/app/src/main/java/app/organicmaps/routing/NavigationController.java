...
import app.organicmaps.sdk.routing.SpeedLimitChange;
...
public class NavigationController implements TrafficManager.TrafficCallback, NavMenu.NavMenuListener
{
  ...
  @NonNull
  private final TextView mEcoAdvice;
                              ...
                              NavMenu.OnMenuSizeChangedListener onMenuSizeChangedListener)
  {
    ...
    mEcoAdvice = mTopFrame.findViewById(R.id.nav_eco_advice);
  ...
  private void updateEcoAdvice(@NonNull RoutingInfo info)
  {
    if (!info.ecoHasData || info.upcomingSpeedLimits == null || info.upcomingSpeedLimits.length == 0)
    {
      UiUtils.showIf(false, mEcoAdvice);
      return;
    }

    // Build a compact advisory string: "70 km/h in 800 m → coast"
    final SpeedLimitChange first = info.upcomingSpeedLimits[0];
    String text = first.speedKmph + " km/h in " + formatEcoDistance(first.distanceMeters);

    // Append recommendation.
    switch (info.ecoRecommendation)
    {
      case Coast:
        text += " → coast";
        break;
      ...
      case BrakeNow:
        ...
        break;
      default:
        // KeepSpeed or ReduceAccelerationTarget – no advisory needed.
        UiUtils.showIf(false, mEcoAdvice);
        return;
    }

    // If it's a sequence (multiple limits close together), append the next one.
    if (info.ecoIsSequence && info.upcomingSpeedLimits.length > 1)
    {
      SpeedLimitChange second = info.upcomingSpeedLimits[1];
      text += " → " + second.speedKmph + " in " + formatEcoDistance(second.distanceMeters);
    }

    mEcoAdvice.setText(text);
    UiUtils.showIf(true, mEcoAdvice);
  }

  private static String formatEcoDistance(double meters)
  {
    if (meters < 1000.0)
      return ((int) Math.round(meters)) + " m";
    double km = meters / 1000.0;
    // Show one decimal for values under 10 km, no decimal above.
    if (km < 10.0)
      return String.format("%.1f", km).replace('.', ',') + " km";
